...
const COURSE_WAYPOINTS = {
  ...
  // (자기교차 재검증) 시작/종료점(37.5473,126.94038)이 관통로에서 살짝 벗어나 있어,
  // OSRM이 그 지점까지 이어지는 막다른 골목(광성로6길 스퍼)을 왕복으로 들렀다 나왔다.
  // 원본(218pt) 좌표만 보면 두 방향 스퍼가 완전히 겹쳐 "교차"로 잡히지 않지만, 30~50개로
  // 단순화(샘플링)하는 과정에서 왕/복 스퍼가 서로 다르게 근사되며 자기교차로 나타났다
  // (course-14와 동일한 유형의 샘플링 아티팩트). 시작/종료점을 그 스퍼가 끝나는 관통로 위
  // 지점(37.547846,126.939634)으로 옮겨 스퍼 자체를 제거해 해소. 실거리 5.9km로 기존
  // 라벨(6.1km)과 3% 이내라 유지.
  'course-9': [
