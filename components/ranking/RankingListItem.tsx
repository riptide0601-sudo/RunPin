...
export function RankingListItem({ entry, course, onPress }: RankingListItemProps) {
  ...
  // mock 코스는 실제 유저별 좋아요 기록이 없다. entry.likeCount는 기간별 랭킹 수치라
  // course.likeCount(현재 총 좋아요 수)와 다른 값이므로, 지금까지 보이던 숫자를 그대로
  // 유지하기 위해 course가 아니라 entry.likeCount를 기준으로 로컬 토글한다.
